...
import { eq, and, notInArray, desc, inArray, sql } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async getDashboardStats(): Promise<any> {
    const totalEmployees = await db.select({ count: sql<number>`count(*)` }).from(employees).where(eq(employees.isDeleted, false));
    const activeEmployees = await db.select({ count: sql<number>`count(*)` }).from(employees).where(and(eq(employees.isDeleted, false), eq(employees.currentStatus, "على رأس عمله")));
    const onlineUsers = await db.select({ count: sql<number>`count(*)` }).from(users).where(eq(users.isOnline, true));
    
    // Status breakdown
    const statusBreakdown = await db.select({
      status: employees.currentStatus,
      count: sql<number>`count(*)`
    }).from(employees).where(eq(employees.isDeleted, false)).groupBy(employees.currentStatus);

    // Certificate breakdown
    const certBreakdown = await db.select({
      cert: employees.certificateType,
      count: sql<number>`count(*)`
    }).from(employees).where(eq(employees.isDeleted, false)).groupBy(employees.certificateType);

    // Category breakdown
    const categoryBreakdown = await db.select({
      category: employees.category,
      count: sql<number>`count(*)`
    }).from(employees).where(eq(employees.isDeleted, false)).groupBy(employees.category);
